...
class LightPathCycle {
	List<List<Integer>> input_dir = Arrays.asList(Arrays.asList(1, 0), Arrays.asList(-1, 0), Arrays.asList(0, -1),
			Arrays.asList(0, 1));
	List<List<Integer>> s_dir = Arrays.asList(Arrays.asList(1, 0), Arrays.asList(-1, 0), Arrays.asList(0, -1),
			Arrays.asList(0, 1));
	List<List<Integer>> l_dir = Arrays.asList(Arrays.asList(0, 1), Arrays.asList(0, -1), Arrays.asList(1, 0),
			Arrays.asList(-1, 0));
	List<List<Integer>> r_dir = Arrays.asList(Arrays.asList(0, -1), Arrays.asList(0, 1), Arrays.asList(-1, 0),
			Arrays.asList(1, 0));
				...
				for (int x = 0; x < 4; x++) {
					if (!(pos_cycle[pos[0]][pos[1]][x])) {
						...
						while (!(Arrays.equals(pos, start_pos) && Arrays.equals(first_dir, move_dir))) {
							pos_cycle[pos[0]][pos[1]][input_dir
									.indexOf(Arrays.stream(move_dir).boxed().collect(Collectors.toList()))] = true;
							first_dir = input_dir.get(x).stream().mapToInt(Integer::intValue).toArray();
							// System.out.println(Arrays.toString(move_dir) + " " + Arrays.toString(pos) +
							// grid[pos[0]].charAt(pos[1]) + " "
							// +Arrays.toString(pos_cycle[pos[0]][pos[1]]));
							dx = move_dir[0];
							dy = move_dir[1];
							...
							if (grid[pos[0]].charAt(pos[1]) == 'S') {
								move_dir = s_dir
										.get(input_dir
												.indexOf(Arrays.stream(move_dir).boxed().collect(Collectors.toList())))
